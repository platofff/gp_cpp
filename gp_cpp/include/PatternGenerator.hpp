...
public:
  PatternGenerator(const size_t width, const size_t height,
    ...
    for (size_t i = 0; i < nCollections; i++) {
      ...
      std::vector<BitImage<T>> col, s_col, o_col;
      ...
      for (size_t j = 0; j < nImages; j++) {
        const OffsettedImgAlpha img(collections[i][j], offset);
        const OffsettedImgAlpha s_img(collections[i][j], collection_offset);

        col.emplace_back(img);
        s_col.emplace_back(s_img);
        o_col.emplace_back(collections[i][j]);
        bo.emplace_back(
            std::make_pair(img.getBaseOffset(), s_img.getBaseOffset()));
      }

      this->rCollections.push_back(std::move(col));
  ...
  std::vector<std::vector<std::optional<std::vector<Point>>>> generate() {
    ...
    AnnealingSimulator<T> sim(0.99, 2000);

    for (const auto &[collection_idx, img_idx] : indices) {
      const auto _p = sim.optimimizePlacement(
          cCanvases[collection_idx], oCollections[collection_idx][img_idx]);
      if (_p.has_value()) {
        const auto &p = _p.value();
        const auto [bo, sbo] = this->baseOffsets[collection_idx][img_idx];
        result[collection_idx][img_idx] = {
            p, Point{p.x - this->width, p.y}, Point{p.x, p.y - this->height},
            Point{p.x - this->width,
                  p.y - this->height}}; // TODO: implement other points only
                                        // if needed
