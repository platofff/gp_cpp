    ...
    for (const auto& pa : areas) {
        ...
        for (ptrdiff_t row = 0; row < height; ++row) {
            ...
            while (num_bits > 0) {
                ...
                size_t chunk_size = std::min(num_bits, std::min(canvas_remaining_bits, image_remaining_bits));
    ...
    for (const auto &pa : areas) {
        ...
        for (ptrdiff_t row = 0; row < height; ++row) {
            ...
            while (num_bits > 0) {
                ...
                canvas_block = (canvas_block & clear_mask) | (combined_chunk << canvas_bit_in_block);
